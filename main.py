############################################# IMPORTING ################################################
# Importing necessary libraries for GUI, Image Processing, and Data Handling
import tkinter as tk             # Tkinter for GUI development
from tkinter import ttk          # Additional Tkinter modules for better UI
from tkinter import messagebox as mess 
from tkinter import PhotoImage
import tkinter.simpledialog as tsd   # Dialogs for user input
from PIL import Image, ImageTk       # PIL for handling images
from PIL import Image # type: ignore
import cv2,os         # OpenCV for face recognition # OS module for file operations
...
import time                          # Date and time operations
import smtplib                       # For sending emails
from email.mime.multipart import MIMEMultipart  # Email handling
...
##################################################################################
# Function to update the clock in the GUI
def tick():
...
def contact():
    mess._show(title='Contact us', message="Please contact us on : 'syedrizvi062005@gmail.com' ")
...
def change_pass():
    ...
    cancel=tk.Button(master,text="Cancel", command=master.destroy ,fg="black"  ,bg="red" ,height=1,width=20 , activebackground = "white" ,font=('comic', 10, ' bold '))
    cancel.place(x=250, y=120)
    save1 = tk.Button(master, text="Save", command=save_pass, fg="black", bg="#00fcca", height = 1,width=20, activebackground="white", font=('comic', 10, ' bold '))
    save1.place(x=60, y=120)
    master.mainloop()
    ...
    if ((name.isalpha()) or (' ' in name)):
        ...
        while True:
            ...
            for (x, y, w, h) in faces:
                ...
                # Display Image Counter on Screen
                cv2.putText(img, f'Images Captured: {sampleNum}/100', (20, 40), 
                            cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2)

                # Saving the captured face in the dataset folder TrainingImage (in color)
            ...
            if cv2.waitKey(100) & 0xFF == ord('q'):
                break
            # Break if the sample number reaches 100
            elif sampleNum >= 100:
                break
...
def TrackImages():
    ...
    while True:
        ...
        for (x, y, w, h) in faces:
            ...
            accuracy = 100 - conf
            cv2.putText(im, f"Accuracy: {round(accuracy, 2)}%", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (255, 0, 0), 2)
        ...
        # ✅ Fix: Ensure 'q' key properly exits the loop
...
bg_image = Image.open("background_image1.jpg")
...
frame1.place(relx=0.11, rely=0.17, relwidth=0.36, relheight=0.72)

frame2 = tk.Frame(window, bg="#c79cff")
frame2.place(relx=0.51, rely=0.17, relwidth=0.34, relheight=0.72)

def marquee_text():
    global marquee_string
    marquee_string = marquee_string[1:] + marquee_string[0]  # Rotate text
    message3.config(text=marquee_string)
    message3.after(200, marquee_text)  # Slower speed (300ms instead of 150ms)

# Initialize the marquee text
marquee_string = " | Intelligent Attendance System Utilizing Facial Recognition Technology |  "

message3 = tk.Label(window, text=marquee_string, fg="white", bg="#2d420a",
                    font=('Arial Black', 22, 'bold'), width=60, anchor="w")  # Wider width, left-aligned
message3.place(x=130, y=10)  # Adjust x to fit inside the frame properly
...
datef.place(relx=0.20, rely=0.09)

clock = tk.Label(window, fg="#ff61e5", bg="green", width=20, font=('comic', 15, ' bold '))
clock.place(relx=0.60, rely=0.09)
tick()

head2 = tk.Label(frame2, text="                        For New Registrations                         ", fg="black",bg="#00fcca" ,font=('comic', 17, ' bold ') )
head2.place(x=0,y=-5)

head1 = tk.Label(frame1, text="                       For Already Registered                       ", fg="black",bg="#00fcca" ,font=('comic', 17, ' bold '), padx = 25 )
head1.place(x=0,y=-5)
...
tv.column('#0',width=138)
tv.column('name',width=138)
tv.column('date',width=138)
tv.column('time',width=138)
...
quitWindow = tk.Button(frame1, text="Quit", command=window.destroy  ,fg="black"  ,bg="#eb4600"  ,width=25 ,height=1, activebackground = "white" ,font=('comic', 15, ' bold '))
quitWindow.place(x=100, y=480)
...
def send_email():
    ...
    if not recipient_email:
        mess._show(title='Error', message='Please enter an admin email address.')
        return
    ...
    from_email = "syedrizvi062005@gmail.com"  # Update with your email
    password = "vebq xvni hyou rogz"  # Update with your email password
...
recipient_email_label = tk.Label(frame1, text="Admin's Email", width=31, fg="black", bg="pink",
                                  font=('comic', 9, ' bold '))
...
# Developer Credit Label (Placed at Bottom Center)
developer_label = tk.Label(window, text="Developed by: Syed Rizvi", fg="white", bg="#2d420a", font=("Arial", 14, "bold"))
developer_label.place(relx=0.5, rely=0.95, anchor="center")  # Positioned at bottom center
window.mainloop()
####################################################################################################
